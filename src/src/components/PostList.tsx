import { NewsState } from '@/reducer/newsReducer';
...
const PostList = () => {
  ...
  return (
    <div className="divide-y divide-blue-500 transition-colors dark:divide-gray-200 grid grid-cols-1 gap-x-6 gap-y-10 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-3 xl:gap-x-8">
      ...
      {posts.map((post, idx) => {
        const { _id, toolName, toolShortDescription, websiteUrl, image, pricing, verified, startingPrice, favCount, toolCategories} = post;
        return (
          <li key={_id} className="group transition-colors bg-gray-100 border border-gray-200 rounded-lg shadow-2xl list-none">
            <CustomLink href={websiteUrl}>
              ...
              <div className="px-5 py-3 h-100">
                ...
                <div>
                  {toolCategories.map(aspect => (
                    <span className="inline-flex border border-pink-900 rounded border-radius-4 px-4 py-1 m-1 items-center text-lg my-4 text-pink-600 hover:text-white hover:bg-blue-700">
                      { aspect.categoryName }
                    </span>
                  ))}
                </div>
